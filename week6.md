...
$
J_{test}{(\theta)} = -\frac{1}{m}\sum\limits_{i=1}^{m_{test}}[y_{test}^{(i)}\log{h_{\theta}(x^{(i)}_{test})} + (1-y_{test}^{(i)})\log(1-h_{\theta}(x_{test}^{(i)})]
$
...
**Training error:** $J_{train}(\theta) = \frac{1}{2m}\sum\limits_{i=1}^{m}(h_{\theta}(x^{(i)})-y^{(i)})^2$

**Cross Validation error:** $J_{cv}(\theta) = \frac{1}{2m_{cv}}\sum\limits_{i=1}^{m}(h_{\theta}(x^{(i)}_{cv})-y^{(i)}_{cv})^2$
